...
const dbURI = "mongodb+srv://Azim_Sofi:XpVGlhf5KT21ETiS@cluster0.yfmb6ar.mongodb.net/";
...
// home routes
app.get('/', (req, res) => {
	res.redirect('/index'); //this will redirect page to /index
});
...
//users i.e users route
app.get('/users',(req,res)=>{
	console.log("req made on"+req.url);
	User.find().sort({createdAt:-1})//it will find all data and show it in descending order
		.then(result => { 
			res.render('users', { users: result ,title: 'Home' }); //it will then render users page along with users
		})
		.catch(err => {
			console.log(err);
		});
